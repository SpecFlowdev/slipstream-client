...
Есть и **[клиент для Android](android/)** — те же серверы, те же правила и те
же цифры. Пока он собирается workflow `Android APK`, а не выкладывается в
релизы.
...
### Правила, которые реально режут трафик

Раз релей и так читает адрес назначения из каждого SOCKS5-запроса, он может и **отклонить соединение до того, как пойдёт хоть байт полезной нагрузки**. Это настоящий фильтр, а не индикатор: заблокированный хост получает закрытый сокет.

| Шаблон | Что совпадает |
| --- | --- |
| `example.com` | только этот хост |
| `*.example.com` | его поддомены, но не сам `example.com` — и никогда `notexample.com` |
| `*` | вообще всё, удобно как правило по умолчанию в конце |

Побеждает первое подошедшее правило, поэтому исключение пишется как `allow` выше широкого `block`. Правила применяются к уже поднятому туннелю — переподключаться не нужно, — а любое правило можно выключить, не удаляя.

<div align="center">
  <img src="assets/screenshot-rules.png" alt="Правила маршрутизации" width="100%">
</div>
...
- **История сессий** — каждая завершённая сессия сохраняется с длительностью, итогами и пиковой скоростью, так что видно не только текущую
...
- **Свои обои** — любая картинка, в любой теме, с затемнением и размытием, чтобы интерфейс читался поверх неё. Ниже — сгенерированные примеры; положите своё изображение в `assets/wallpapers/`, и `scripts/screenshots.mjs` снимет панель с ним
...
<table>
  ...
  <tr>
    <td width="50%"><img src="assets/screenshot-wallpaper-sunset.png" alt="Светлые обои"></td>
    <td width="50%"><img src="assets/screenshot-wallpaper-aurora.png" alt="Тёмные обои"></td>
  </tr>
  <tr>
    <td align="center"><strong>Светлая картинка</strong> — сильное затемнение и размытие, чтобы текст побеждал</td>
    <td align="center"><strong>Тёмная</strong> — затемнение почти не нужно</td>
  </tr>
  <tr>
...
**Уже настроили сервер где-то ещё?** У каждого сервера есть кнопка **«Поделиться»** — она превращает его в ссылку `slipstream://` и QR-код: отсканируйте его в [клиенте для Android](android/) или вставьте ссылку в диалог «Добавить по ссылке» в любом из клиентов. Ссылка несёт домен, резолвер, сертификат и настройки; локальный порт и логин/пароль прокси этого устройства в неё никогда не попадают, так что её безопасно показывать на экране или отправлять в чате.
...
DMA-BUF-рендерер WebKitGTK здесь тоже отключён по умолчанию. Если его оставить, окно остаётся пустым, а в консоль сыплется `Failed to create GBM buffer … Invalid argument` — так бывает на проприетарном драйвере NVIDIA, на виртуальных видеокартах и везде, где DRM render node доступен не так, как ожидает WebKit: вместо запасного пути он просто не рисует ничего. Программного пути с избытком хватает для графиков и текста. Если у вас всё в порядке с ускорением, вернуть его можно через `WEBKIT_DISABLE_DMABUF_RENDERER=0`.
...
- Сборки под Android рядом с десктопными, подписанные, а не отладочные
